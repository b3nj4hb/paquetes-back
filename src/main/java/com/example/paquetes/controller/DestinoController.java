...
public class DestinoController {
    ...
    // CREAR
    @PostMapping("/create")
    public ResponseEntity<Destino> save(@RequestBody Destino lib) {
        try {
            Destino l = s.create(lib);
    ...
    // BUSCAR {ID}
    @GetMapping("/read/{id}")
    public ResponseEntity<Destino> search(@PathVariable("id") int id) {
        Destino Destino = s.read(id);
    ...
    public ResponseEntity<Destino> update(@RequestBody Destino l, @PathVariable("id") int id) {
        try {
            Destino li = s.read(id);
            if (li.getIddestino() > 0) {
                li.setCiudad(l.getCiudad());
